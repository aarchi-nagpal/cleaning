    ...
    try:
       ...
       # Extract and standardize date of 'DOB' column
        ...
        if 'u_email_id' in data.columns:
            ...
            if 'U_display_name' in data.columns:
                data['name'] = data['U_display_name'].apply(lambda x: x.strip() if pd.notnull(x) else 'NULL')

            if 'state' in data.columns:
                data['state'] = data['state'].apply(lambda x: x.strip() if pd.notnull(x) else 'NULL')

            if 'Company' in data.columns:
                data['company'] = data['Company'].apply(lambda x: x.strip() if pd.notnull(x) else 'NULL')
            ...
            data.drop(columns=['host', 'domain','DOB' ,'com','U_display_name', 'corr_domain','u_email_id','u_middle_name','u_first_name','u_last_name','salary','Pincode','u_gender','u_emp_tpe','c_city','Company','Pancard'],inplace=True)
...
# Main execution
def main():
    input_file = r'C:\.csv'  # Mention your input file path
    output_file = '.csv'    # Mention your output file path
